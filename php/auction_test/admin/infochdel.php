...
if(empty($_POST) || isset($_POST["reset"])){
	try {
		$sql = "SELECT infos.id, date, goods.name AS good_name, title, content FROM infos LEFT JOIN goods ON infos.good_id = goods.id ORDER BY id ASC";
	...
	try {
		$sql = "SELECT id, name FROM goods ORDER BY id ASC;";
	    foreach ($dbh->query($sql) as $row) {
		$goodName[$row["id"]]  = $row["name"];
		}
	}catch (PDOException $e){
    	echo"SELECTクエリーが失敗しました。".$e->getMessage();
    	die();
...
//画面表示
if(empty($disp)){
	include_once "disp/infochdellist.html";
}
if($disp == "change"){
