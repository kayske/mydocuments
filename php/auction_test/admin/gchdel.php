...
if(empty($_POST) || isset($_POST["reset"])){
	try {
		$sql = "SELECT goods.id, goods.name, img1, img2, img3, description, blandTitle, blandMessage, endDate, flBid, users.name AS user_name FROM goods LEFT JOIN users ON goods.user_id = users.id ORDER BY id ASC;";
		...
		$img[$key][1] = $row["img2"];
		$img[$key][2] = $row["img3"];
    	$description[$key] = stlimit($row["description"],10);
		$blandTitle[$key] = stlimit($row["blandTitle"],10);
		$blandMessage[$key] = stlimit($row["blandMessage"],10);
...
//画面表示
if(empty($disp)){
	include_once "disp/gchdelli.html";
}
if($disp == "change"){
